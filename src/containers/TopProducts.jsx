    ...
    useEffect(() => {
        ...
        const getDataFromEndpoints = async () => {
            ...
            try {
                ...
                const result = await api.get(`${endPoint.url}`, { params: { period: selectedPeriod.value } });
                const topproducts = (await api.get('./data/topProducts.json')).data[selectedPeriod.value];
                            ...
                            {data.map((d, i) => (
                                <ListGroup.Item key={i}>
                                    <div className={styles.itemWrapper}>
                                        <div className={styles.thumb}><img src={`/img/products/${d.product.imgUrl}.png`} alt="" /></div>
                                        <div className={styles.info}>
                                            <div className={styles.title}>{d.product.name}</div>
                                            <div className={styles.price}>${d.product.price}</div>
                                        </div>
                                        <div className={styles.total}>
                                            <div className={styles.amount}>${d.product.salesValue}</div>
                                            <div className={styles.count}>{d.product.soldUnits} Orders</div>
                                        </div>
                                    </div>
                                </ListGroup.Item>
                            ))}
